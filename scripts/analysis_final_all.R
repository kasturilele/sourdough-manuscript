...
#--------- figure 2: multi-species model prediction ---------
library(ggplot2)
library(cowplot)
library(grid)
library(dplyr)
...
#-----------comparison between all observed communities-----------
...
for(n in 0:8){
  #open each new file
  filename <- paste("output288_", n,".csv",sep = "" )
  #open the data
...
LOO_model_main <- read.csv("/Users/kasturilele/Documents/cluster/endpoints_new_OG_model_288.csv", header = T, row.names = 1)
...
plot_mindist <- ggplot(data = mindist_combi) +
  ...
  theme(axis.text = element_text(size = 16),
        ...
        legend.position = 'none')
plot_mindist
ggsave("~/Documents/figures_dump/LOO/all_mindist2.pdf", width=7.20, height=2.40, units = "in")

#------------- supplementary figure 4 - serial transfer model vs original model ------------

#open the original model file and get the data to build model 
new_data_main <- as.data.frame(read.csv("~/Documents/cluster/endpoints_new_OG_model_288.csv", header = T, row.names = 1))
new_data_plot <- as.data.frame(read.csv("~/Documents/cluster/endpoints_new_OG_model_288.csv", header = T))
new_data_main <- round(new_data_main)
new_data_plot[,c(2:10)] <- round(new_data_plot[,c(2:10)]) #to get rid of the one spurious negative value that somehow snuck into the data
...
#plot relative abundance for serial dilution model using the clustering from original model
# change data from wide format to long format
colIDs <- colnames(new_data)
new_data_long <- new_data_plot %>% pivot_longer(cols=all_of(colIDs),
                                                names_to='Species',
...
p_ordered #1242X261
ggsave("~/Documents/for_paper/figures_dump/3_model_clustered_serial.pdf", width=12.42, height=2.60, units = "in")
