...
t_int = np.linspace(0,288,600) #defines a vector of timepoints to solve over
...
# 9-species equation solver - loops over all 
...
#new code that can leave one species out automatically by setting each species' abundance to 0
    seed_start = 1998
    ...
    with open(f'output288_{sI}.csv', 'w') as f:
